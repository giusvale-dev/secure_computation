\begin{abstract}
    This work shows an experiment where a poisoning attack is used on a binary image classifier trained with the CIFAR-10 dataset. The model is trained to recognize images as either "Bird" or "Cat". The experiment is based on the Poison Frogs attack (Algorithm 1). The main goal is to change a "Cat" image so that the model wrongly predicts it as a "Bird".
